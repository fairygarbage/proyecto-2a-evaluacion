...
def add_order():
    """Agrega una nueva orden a la base de datos en el nodo dos y varios items en el nodo uno."""
    try:
        ...
        # Permitir valores NULL en sales_rep_id y promotion_id
        sales_rep_id = data.get('sales_rep_id', None)
...
def delete_order():
    try:
        ...
        deleteOrderId = request.form['deleteOrderId']

        # Insertar los datos del nuevo producto en la tabla
...
def add_order_item():
    """Agrega un nuevo order_item a la base de datos."""
    try:
        connection = connect_to_node1() 
        cursor = connection.cursor()
...
@app.route('/delete_order_items', methods=['POST'])
